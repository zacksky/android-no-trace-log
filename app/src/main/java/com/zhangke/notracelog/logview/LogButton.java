        ...
        for (int i = 0, len = fields.length; i < len; i++) {
            ...
            try {
                ...
                if (fields[i].getType() == this.getClass()) {
                    ((LogButton) oo).setObject(f);
                    if (TextUtils.isEmpty(((LogButton) oo).getLogName())) {
                        if (f instanceof BaseActivity) {
                            ((LogButton) oo).setLogName(((BaseActivity) f).getClass().getSimpleName() + "-" + varName);
                        } else if (f instanceof BaseFragment) {
                            ((LogButton) oo).setLogName(((BaseFragment) f).getClass().getSimpleName() + "-" + varName);
                        }
                    }
                }
