        ...
        for (int i = 0, len = fields.length; i < len; i++) {
            ...
            try {
                ...
                if (fields[i].getType() == this.getClass()) {
                    ((LogLinearLayout) oo).setObject(f);
                    if (TextUtils.isEmpty(((LogLinearLayout) oo).getLogName())) {
                        if (f instanceof BaseActivity) {
                            ((LogLinearLayout) oo).setLogName(((BaseActivity) f).getClass().getSimpleName() + "-" + varName);
                        } else if (f instanceof BaseFragment) {
                            ((LogLinearLayout) oo).setLogName(((BaseFragment) f).getClass().getSimpleName() + "-" + varName);
                        }
                    }
                }
